...
\documentclass[preview, border={0pt 2pt 1pt 1pt}, varwidth=7.5cm]{standalone} % border options are {left bottom right top}
...
\begin{document}
    ...
    % Suppose the base rate of the disease is \(\Pr\left[ d \right] = 0.1\), 
    % i.e., out of \(100\) foetuses \(10\) will be afflicted.
    % \newline

    % The test detects the disease with probability \(\Pr\left[ + \mid d \right] = 0.9\),
    % i.e., it catches \(9\) out of the \(10\) cases.
    % \newline

    % The false positive rate is \(\Pr\left[ + \mid \lnot d \right] = 0.1\),
    ...
    % Note that if the disease is rarer, say \(\Pr\left[ d \right] = 0.05\),
    % then the same test parameters give us:
    % \[
    %     \Pr\left[ d \mid + \right] \approx 0.32.
    % \]
