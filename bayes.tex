...
\begin{document}
    ...
    % For the MaterniT21 case, 
    % recall that one positive result gave us:
    % \[
    %     \Pr\left[ d \mid + \right] = 0.64.
    % \]
    % With two positive test results we can be almost certain:
    % \[
    %     \Pr\left[ d \mid +, + \right] = 0.9998.
    % \]
    % With three positive test reults we can be certainer:
    % \[
