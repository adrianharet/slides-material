...
\documentclass[preview, border={0pt 5pt 3pt 1pt}, varwidth=11cm]{standalone} % border options are {left bottom right top}
...
\begin{document}
  ...
  % In our case, each independent random variable \(v_i\) 
    ...
    % Suppose we add two voters with competences \(p_4 = \nicefrac{9}{16}\) and \(p_5 = \nicefrac{17}{32}\) to the previous group.\\

    % We now have a group of five voters with competences 
    % \(\bm{p} = \Big(1, \nicefrac{3}{4}, \nicefrac{5}{8}, \nicefrac{15}{16}, \nicefrac{16}{32} \Big)\).\\
